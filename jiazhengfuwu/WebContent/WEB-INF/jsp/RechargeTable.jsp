...
<body>
	...
	<form action="recharge/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="toolbar">
					...
					<li class="click"><font color="#0099cc"><a href="recharge/exportAll">导出表格</a>
					</font></li>
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${rechargeList.datas }" var="recharge"
						varStatus="vg">
						<tr>
							<td><c:out value="${recharge['userInfo']['userName'] }"></c:out></td>
							<td><c:out value="${recharge['money'] }"></c:out></td>
							...
							<td><c:out value="${recharge['createtime'] }"></c:out></td>
							<td><a href="javascript:;"
								onclick="deletecheck(${recharge['id']})" class="tablelink">
									删除</a></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="recharge/informationLoad?pageNow=${pager.pageNow }&name=${recharge['name']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="recharge/informationLoad?pageNow=${pager.pageNow-1 }&name=${recharge['name']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
						...
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								<li class="paginItem current"><a
									href="recharge/informationLoad?pageNow=${vs.count }&name=${recharge['name']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
										href="recharge/informationLoad?pageNow=${count }&name=${recharge['name']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
										href="recharge/informationLoad?pageNow=${count }&name=${recharge['name']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="recharge/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
