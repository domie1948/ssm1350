...
<body>
	...
	<form action="user/employeeQuery" method="POST"
		onsubmit="return check();">
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>地区</label><input type="text" class="scinput1"
						name="address" value="${user['address'] }"></li>
					<li><label>年龄</label><input type="text" class="scinput1"
						name="age" value="${user['age'] }"></li>
					<li><label>性别</label>
					<div class="vocation">
						<select name="sex" class="select3">
							<option value="">请选择</option>
							<c:if test="${user['sex'] eq 1}" var="t1">
								<option value="1" selected="selected">男</option>
							</c:if>
							<c:if test="${!t1}">
								<option value="1">男</option>
							</c:if>
							<c:if test="${user['sex'] eq 2}" var="t2">
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="toolbar">
					...
					<li class="click"><font color="#0099cc"><a href="user/exportAllEmployee?address=${user['address'] }&sex=${user['sex']}&age=${user['age']}&roleId=2">导出表格</a>
					</font></li>
			...
			<table class="imgtable">
				...
				<tbody>
					<c:if test="${message ne null }" var="null">
						<tr>
							<td colspan="10" align="center">${message }</td>
						</tr>
					</c:if>
					<c:forEach items="${pager.datas }" var="user" varStatus="vg">
						<tr>
							<td>
								<div id="images">
								<img src="upload/upload/${user['url'] }">
								</div>
							</td>
							...
							<td><c:out value="${user['flag'] }"></c:out></td>
							<td><c:out value="${user['vip']['level'] }"></c:out></td>
							<td>
								<c:if test="${user['vip']['status'] eq 0}">
									<c:out value="未成为会员" />
								</c:if>
								<c:if test="${user['vip']['status'] eq 1}">
									<c:out value="正常" />
								</c:if>
								<c:if test="${user['vip']['status'] eq 2}">
									<c:out value="已过期" />
								</c:if>
							</td>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/employeeQuery?pageNow=${pager.pageNow }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/employeeQuery?pageNow=${pager.pageNow-1 }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/employeeQuery?pageNow=${vs.count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/employeeQuery?pageNow=${count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/employeeQuery?pageNow=${count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${count}</a></li>
