...
<body>
	...
	<form action="user/masterQuery" method="POST"
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="toolbar">
					...
					<li class="click"><font color="#0099cc"><a href="user/exportAllMaster?address=${user['address'] }&sex=${user['sex']}&age=${user['age']}&roleId=3">导出表格</a>
					</font></li>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/masterQuery?pageNow=${pager.pageNow }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/masterQuery?pageNow=${pager.pageNow-1 }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/masterQuery?pageNow=${vs.count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/masterQuery?pageNow=${count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/masterQuery?pageNow=${count }&address=${user['address']}&age=${user['age']}&sex=${user['sex']}">${count}</a></li>
